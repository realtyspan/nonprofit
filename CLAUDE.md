...
- **Multi-tenant**: every org-scoped table has an `orgId`; every route filters by `req.user.orgId`. Permissions are per-module (`bell-jar`, `rentals`, `calendar`, `raffle`) with tiers `Owner > Admin > Helper > Viewer`.
