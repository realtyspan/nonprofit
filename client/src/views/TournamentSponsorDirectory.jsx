...
// ever recorded, live (both funnel into the same TournamentSponsorContact
// table, see tournaments.js's /sponsors route). This is the "who do we
// reach out to for next year's sponsorship" screen; TournamentSponsors.jsx
// stays scoped to one tournament's own sponsorships and their payment
// status. Direct port of GolfSponsorDirectory.jsx.
export default function TournamentSponsorDirectory() {
  ...
  function refresh() {
    api.listTournamentSponsorDirectory().then(setSponsors).catch((err) => setError(err.message));
  }
  useEffect(refresh, []);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ ...card, padding: 0, overflow: "hidden" }}>
        <DataList
          rows={filtered}
          emptyMessage={search ? "No sponsors match that search." : "No sponsors yet — they'll show up here once one is added to a tournament."}
          columns={[
...
function EditSponsorModal({ sponsor, onCancel, onSaved }) {
  ...
  async function submit(e) {
    ...
    setError("");
    try {
      await api.updateTournamentSponsorContact(sponsor.id, { companyName: companyName.trim(), contactName: contactName.trim(), email: email.trim(), phone: phone.trim() });
      onSaved();
    } catch (err) {
