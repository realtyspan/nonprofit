...
// same convention as GolfCheckIn.jsx/RaffleCheckIn.jsx. Direct port of
// GolfCheckIn.jsx.
const POLL_MS = 4000;
...
export default function TournamentCheckIn({ tournament }) {
  ...
  function refresh() {
    ...
    api.listTournamentCheckIns(tournament.id).then(setCheckIns).catch(() => {});
    api.searchTournamentCheckIn(tournament.id).then(setSearchable).catch(() => {});
  }
  ...
  async function submit(e) {
    ...
    setNotice("");
    try {
      const res = await api.toggleTournamentCheckIn(tournament.id, target.id);
