...
function TeamCard({ team, tournament, onChanged }) {
  ...
  async function markPaid(teamPlayerId, paymentMethod) {
    ...
    setError("");
    try {
      await api.markTournamentTeamPaid(tournament.id, team.id, { teamPlayerIds: [teamPlayerId], paymentMethod });
      onChanged();
    } catch (err) {
  ...
  async function removePlayer(teamPlayerId) {
    ...
    setError("");
    try {
      await api.removeTournamentTeamPlayer(tournament.id, team.id, teamPlayerId);
      onChanged();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  async function removeTeam() {
    ...
    setError("");
    try {
      await api.deleteTournamentTeam(tournament.id, team.id);
      onChanged();
    } catch (err) {
      setError(err.message);
    } finally {
...
function AddPlayerForm({ tournament, team, onCancel, onAdded }) {
  ...
  async function submit(e) {
    ...
    setError("");
    try {
      await api.addTournamentTeamPlayer(tournament.id, team.id, { name: name.trim(), email: email.trim(), phone: phone.trim(), isCaptain });
      onAdded();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  return (
    <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 8, padding: 10, background: "#f7f4ec", borderRadius: 8 }}>
      <DirectorySearchField
        ...
        searchFn={api.searchTournamentPlayers}
        renderResult={(p) => (
          <div>
...
function AddTeamModal({ tournament, onCancel, onCreated }) {
  ...
  async function submit(e) {
    ...
    setError("");
    try {
      await api.createTournamentTeam(tournament.id, { teamName: teamName.trim(), players });
      onCreated();
    } catch (err) {
      setError(err.message);
    } finally {
