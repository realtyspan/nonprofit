...
// The standing, tournament-independent player list — direct port of
// GolfPlayerDirectory.jsx. Every player ever recorded across every
// tournament of every type lands in this one table.
export default function TournamentPlayerDirectory() {
  ...
  function refresh() {
    api.listTournamentPlayers().then(setPlayers).catch((err) => setError(err.message));
  }
  useEffect(refresh, []);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ ...card, padding: 0, overflow: "hidden" }}>
        <DataList
          rows={filtered}
          emptyMessage={search ? "No players match that search." : "No players yet — they'll show up here once someone registers."}
          columns={[
...
function EditPlayerModal({ player, onCancel, onSaved }) {
  ...
  async function submit(e) {
    ...
    setError("");
    try {
      await api.updateTournamentPlayer(player.id, { name: name.trim(), email: email.trim(), phone: phone.trim() });
      onSaved();
    } catch (err) {
