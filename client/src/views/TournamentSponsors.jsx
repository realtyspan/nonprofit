...
// Sponsorship management — the parallel fundraising track alongside team
// registration. Admin-entered only in this pass; a public sponsor-inquiry
// form is a later build step. Direct port of GolfSponsors.jsx.
export default function TournamentSponsors({ tournament }) {
  ...
  function refresh() {
    ...
    api.listTournamentSponsorships(tournament.id).then(setSponsorships).catch((err) => setError(err.message));
  }
  ...
  async function confirmInquiry(id) {
    try {
      await api.confirmTournamentSponsorship(tournament.id, id);
      refresh();
    } catch (err) {
  ...
  async function deleteSponsorship(s) {
    ...
    try {
      await api.deleteTournamentSponsorship(tournament.id, s.id);
      refresh();
    } catch (err) {
      setError(err.message);
    }
  ...
  async function togglePaid(s) {
    try {
      await api.updateTournamentSponsorship(tournament.id, s.id, { paid: !s.paid, paymentMethod: s.paid ? undefined : "check" });
      refresh();
    } catch (err) {
      setError(err.message);
    }
...
function AddSponsorForm({ tournament, onCancel, onAdded }) {
  ...
  async function submit(e) {
    ...
    setError("");
    try {
      await api.createTournamentSponsorship(tournament.id, form);
      onAdded();
    } catch (err) {
      setError(err.message);
    } finally {
    ...
    <div style={{ ...card, display: "flex", flexDirection: "column", gap: 12, maxWidth: 480 }}>
      ...
      <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <DirectorySearchField
          ...
          searchFn={api.searchTournamentSponsors}
          renderResult={(s) => (
            <div>
        ...
        <textarea style={{ ...inputStyle, minHeight: 60, resize: "vertical", fontFamily: "inherit" }} placeholder="Benefits (e.g. sign at the venue, logo on program)" value={form.benefitsText} onChange={(e) => set("benefitsText", e.target.value)} />
        {error && <div style={{ color: colors.danger, fontSize: 12.5 }}>{error}</div>}
