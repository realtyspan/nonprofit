...
// this year's. Direct port of golfSponsorEmail.js, generalized off "golf".
// Same visual language as tournamentKickoffEmail.js (dark hero band, dark
// contact block, org's own flyer colors) with sponsor-specific copy:
// states last year's tier/amount where known, as a natural "renew at the
// same level" prompt. There's no public self-serve sponsorship form in
// this app — sponsors are recruited by relationship, not a web form (see
// the admin-entered sponsorship flow in tournaments.js) — so the call to
// action is to reply or call, with a link to the tournament's public page
// for details rather than a signup button that doesn't exist.
...
function tournamentSponsorEmailHtml({ org, tournament, registerUrl, recipientName, lastTierName, lastAmount, unsubscribeUrl }) {
      ...
      <table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;background:#faf8f2;border-radius:10px;overflow:hidden;">
        ...
        <tr><td style="background:${primary};padding:22px 32px;text-align:center;">
          ...
          <div style="font-size:10.5px;color:#ffffff;opacity:0.65;margin-top:14px;line-height:1.6;">
            You're receiving this because you've sponsored a past ${org.name || "our"} tournament.<br>
            ${unsubscribeUrl ? `<a href="${unsubscribeUrl}" style="color:#ffffff;">Unsubscribe from tournament emails</a>` : `<span>Unsubscribe from tournament emails</span>`}
          </div>
        </td></tr>
...
module.exports = { tournamentSponsorEmailHtml };
