// AI-assisted fallback for a historical tournament spreadsheet that doesn't
// match any shape interpretPlayerRows/interpretSponsorRows
// (tournamentHistoricalImport.js) recognizes — direct port of
// golfHistoricalImportAi.js, reworded off "golf tournament". Same
// philosophy as labelScan.js's game-label reader: the result only ever
// pre-fills a review screen the org confirms or corrects before anything
// saves, so an imperfect read here is a UX cost, not a data-integrity risk
// the way a silent misimport would be.
...
const SPONSOR_PROMPT_HEADER = `Below is a spreadsheet of tournament sponsors, one row per line, cells separated by " | " (a "·" means a blank cell). It does NOT necessarily match any standard format — column meanings vary by organization. Your job is to read it like a person would and extract every sponsor into a flat JSON array.
...
async function extractPlayersFromRows(rawRows, orgId) {
  ...
  const rows = await callClaude(PLAYER_PROMPT_HEADER + rowsToText(rawRows), 8000, { orgId, feature: "tournament-historical-import-players" });
  return rows
...
async function extractSponsorsFromRows(rawRows, orgId) {
  ...
  const rows = await callClaude(SPONSOR_PROMPT_HEADER + rowsToText(rawRows), 8000, { orgId, feature: "tournament-historical-import-sponsors" });
  return rows
