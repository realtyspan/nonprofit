...
const { publishGolfTournament, removeCalendarEventFor } = require("../lib/calendarSync");
...
router.post("/stripe-connect/onboard", requireOwnerOrGolfAdmin, async (req, res) => {
  ...
  if (!connect?.stripeAccountId) {
    ...
    try {
      ...
      // A concurrent onboard call (double-click) already created the connect
      // row with its own Stripe account. The account we just made is an
      // orphan — delete it from Stripe so the org isn't left with two, and
      // fall through on the one that won.
...
router.patch("/tournaments/:tournamentId", requirePermission("golf", "Admin"), requireActiveGolfTournament, async (req, res) => {
  ...
  if (updated.status === "open") {
    const org = await prisma.organization.findUnique({ where: { id: req.user.orgId } });
    await publishGolfTournament(req.user.orgId, updated, org);
  }
...
router.post("/tournaments/:tournamentId/close", requirePermission("golf", "Admin"), async (req, res) => {
  ...
  await removeCalendarEventFor("golf-tournament", req.golfTournament.id);
  res.json(updated);
});
...
router.post("/tournaments/:tournamentId/teams/:teamId/mark-paid", requirePermission("golf", "Admin"), requireActiveGolfTournament, async (req, res) => {
  ...
  const result = await prisma.golfTeamPlayer.updateMany({
    where: { id: { in: teamPlayerIds }, teamId: req.golfTeam.id, orgId: req.user.orgId, paymentStatus: { not: "paid" } },
